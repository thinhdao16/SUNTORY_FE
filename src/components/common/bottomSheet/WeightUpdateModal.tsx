...
const WeightUpdateModal: React.FC<WeightUpdateModalProps> = ({
    ...
    const displayValue = unit === 'kg' ? valueKg : valueLbs;
    ...
    // Helper function to update weight with boundary checking
    ...
    const applyMomentum = () => {
        ...
        const momentumLoop = (currentTime: number) => {
            ...
            if (currentWeight === 0 && currentVelocity > 0) {
            ...
            const weightSteps = Math.floor(Math.abs(accumulatedDistance) / sensitivity);
            if (weightSteps > 0) {
    ...
    const handleRulerTouchMove = (e: React.TouchEvent) => {
        ...
        // Weight updates with smooth sensitivity
        ...
        if (steps > 0 && Math.abs(deltaX) >= sensitivity) {
            ...
            if (direction < 0 && !canDecrease()) {
                ...
                updateWeight(direction); // Use direction directly now
                setStartX(newX);
                setTotalDistance(0);
                setSmoothOffset(0);
            }
    ...
    const handleSave = async () => {
        ...
        try {
            const payload: UpdateHealthConditionV2Payload = {
                weight: unit === 'kg' ? valueKg : valueLbs,
                weightUnitId: unit === 'kg' ? units?.measurementUnits.find((a: any) => a.symbol === 'kg')?.id : units?.measurementUnits.find((a: any) => a.symbol === 'lbs')?.id,
                allergies:null,
                lifestyleId: null,
                            ...
                            <div className="flex-1 overflow-y-auto" style={{
                                ...
                                <div className="px-6 pt-2 flex flex-col items-center gap-4">
                                    ...
                                    <div className="w-full mt-6">
                                        {/* tick bar with animation - shows actual weight values */}
