...
import { useImageUploadState } from '../CreateFeed/hooks/useImageUploadState';
import { useImageUploadHandlers } from '../CreateFeed/hooks/useImageUploadHandlers';
import { useAudioUploadState } from '../CreateFeed/hooks/useAudioUploadState';
...
const EditFeed: React.FC = () => {
    ...
    const { postCode } = useParams<EditFeedParams>();
    ...
    useEffect(() => {
            ...
            try {
                ...
                if (post.media && post.media.length > 0) {
                    const mediaItems = post.media.map((media, index) => {
                        // Create a dummy File object for existing media
                    ...
                    if (audioMedia) {
                        // Note: We can't recreate the Blob from URL, so we'll just show the audio info
                        setAudioServerUrl(audioMedia.urlFile);
                        setAudioFilename(audioMedia.fileName || '');
                        setAudioDuration(0); // We don't have duration info
                    }
                }
            } catch (error) {
        ...
        loadPost();
    }, [postCode, history, setImages, setAudioServerUrl, setAudioFilename, setAudioDuration]);
