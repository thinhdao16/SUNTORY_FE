...
const ChatInputBar: React.FC<ChatInputBarProps> = (props) => {
  ...
  const [isEditingTranslate, setIsEditingTranslate] = useState(false);
  ...
  useEffect(() => {
    ...
    if (isEditingTranslate) return; // do not auto-translate while user is editing translation output
    if (!debouncedSource.trim()) return;
    void onTranslate(debouncedSource);
  }, [translateActionStatus, debouncedSource, selectedLanguageSocialChat, isComposing]);
    ...
    <div ref={containerRef} className={`pb-1 bg-white relative ${hasReachedLimit ? "pointer-events-none opacity-50" : ""}`}>
        ...
        <div className="flex items-end gap-4 px-4 py-2 border-t-1 border-netural-50">
          <div className="relative flex-1">
            ...
            <NormalizedTextarea
              ...
              onValueChange={(v) => handleChangeTranslate(v)}
              rows={1}
              ...
              onFocus={() => { onFocus("translate"); setIsEditingTranslate(true); }}
              onClick={() => onFocus("translate")}
              onBlur={() => { onBlur("translate"); setIsEditingTranslate(false); }}
              onCompositionStart={() => { composingRef.current = true; setIsComposing(true); handleTypingActivity(); }}
              onCompositionEnd={(e) => { composingRef.current = false; setIsComposing(false); e.currentTarget.value.trim() ? handleTypingActivity() : forceTypingOff(); }}
              onKeyDown={(e) => {
                ...
                if (ev.key === "Enter" && !ev.shiftKey) {
                  ...
                  if (messageTranslate.trim() && !isSending && !isTranslating && !isComposing) {
          ...
          <div className="gap-4 flex items-center h-fit pb-2">
            ...
            {( (messageValue.trim() || messageTranslate.trim() || (pendingFiles && pendingFiles.length > 0)) && translateActionStatus ) && (
              <button
                ref={sendBtnRef}
                type="button"
                disabled={hasReachedLimit || isSending || isTranslating}
                onMouseDown={preventBlur}
                onTouchStart={preventBlur}
                onClick={(e) => {
                  if (isSending || isTranslating) return;
              ...
              >
                {(isTranslating || isSending) ? (
                  <div className="w-5 h-5 rounded-full border-2 border-blue-600 border-t-transparent animate-spin" aria-label="loading" />
                ) : (
                  <SendIcon />
                )}
              </button>
      ...
      <div className="flex items-end gap-4 transition-all py-2 px-4" onMouseDown={preventBlur}>
        <NormalizedTextarea
          ...
          ref={messageRef}
          value={messageValue}
          onValueChange={(v) => handleChangeInput(v)}
          rows={1}
