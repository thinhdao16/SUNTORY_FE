...
export const usePendingFilesHandlers = ({
    ...
    const handleSendMessage = useCallback(
        async (e: any, field: string, force?: boolean) => {
            await clearTypingAfterSend();

            if (chatPendingFiles.length > 0) {
                const uploadingFiles = chatPendingFiles.filter(
                    (f: PendingFile) => f.isUploading
                );
                ...
                // const hasMessAndFiles =
                //     chatPendingFiles.length > 0 &&
                //     (!isEmptyText(messageValue) || !isEmptyText(messageTranslate));
                // const uploadedFiles = chatPendingFiles.filter(
                //     (f: PendingFile) => !f.isUploading && !f.error && f.serverUrl
                // );
                // if (uploadedFiles.length > 0 && hasMessAndFiles) {
                //     clearPendingFiles();
    ...
    const handleOpenGallery = useCallback(() => {
        ...
        input.onchange = async (e) => {
            ...
            if (invalidFiles.length > 0) {
                showToast(
                    `${invalidFiles.length} file không được hỗ trợ: ${invalidFiles
                        .map((f) => f.name)
                        .join(", ")}`,
                    4000,
                    "warning"
